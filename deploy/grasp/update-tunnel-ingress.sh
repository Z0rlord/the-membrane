#!/usr/bin/env bash
# Add membrane-grasp.dojopop.live → localhost:7334 to the dojopop Cloudflare Tunnel.
#
...
GRASP_HOST="${MEMBRANE_GRASP_HOST:-membrane-grasp.dojopop.live}"
GRASP_PORT="${MEMBRANE_GRASP_PORT:-7334}"
...
python3 - /tmp/cf-tunnel-current.json /tmp/cf-tunnel-merged.json "$GRASP_HOST" "$GRASP_PORT" <<'PY'
import json, sys
...
echo "==> DNS CNAME membrane-grasp (zone ${DOJOPOP_ZONE_ID})..."
DNS_RESP=$(curl -sS -X POST "https://api.cloudflare.com/client/v4/zones/${DOJOPOP_ZONE_ID}/dns_records" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"type\":\"CNAME\",\"name\":\"membrane-grasp\",\"content\":\"${TARGET}\",\"proxied\":true}")
python3 -c "
import json, sys
d = json.loads(sys.argv[1])
if d.get('success'):
    print('  membrane-grasp: ok')
else:
    errors = d.get('errors') or []
    msg = errors[0].get('message', 'failed') if errors else 'failed'
    if 'already exists' in msg.lower():
        print('  membrane-grasp: already exists (ok)')
    else:
        print('  membrane-grasp:', msg)
        sys.exit(1)
" "$DNS_RESP"

echo ""
echo "Verify:"
echo "  curl -s -H 'Accept: application/nostr+json' https://${GRASP_HOST} | python3 -m json.tool"
